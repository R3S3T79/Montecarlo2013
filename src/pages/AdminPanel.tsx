...
// Data creazione chat: 2025-08-19 (rev: aggiunta eliminazione utente + approvazione/cambio ruolo + reinvio welcome)
...
export default function AdminPanel() {
  ...
  // Reinvio mail di benvenuto
  const resendWelcome = async (email: string) => {
    if (processing.has(email)) return;
    setProcessing((prev) => new Set(prev).add(email));
    try {
      const res = await fetch("/.netlify/functions/resend-welcome", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        alert(`Mail di benvenuto reinviata a ${email}`);
      }
    } catch (err: any) {
      console.error(err);
      alert("Errore di connessione: " + err.message);
                  ...
                  <tr key={u.id} className="hover:bg-gray-50 border-b">
                    ...
                    <td className="p-2 flex items-center space-x-2 overflow-x-auto">
                      ...
                      <button
                        ...
                        disabled={processing.has(u.email)}
                        className="px-3 py-1 border rounded inline-flex items-center text-sm text-blue-700 hover:bg-blue-50 bg-white"
                        title="Reinvia mail di benvenuto"
                      >
