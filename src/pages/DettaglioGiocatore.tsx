...
// 2. Tipi del dettaglio giocatore
interface Giocatore {
...
// 3. Componente e selezione delle stagioni
const TUTTE_LE_STAGIONI = 'tutte';
...
rigoriSubiti: 0,
  ...
  const [loadingStatistiche, setLoadingStatistiche] = useState(false);
  const richiestaStatistiche = useRef(0);

  // 4. Dettaglio presenze
const [mostraPresenze, setMostraPresenze] = useState(false);
const [partitePresenze, setPartitePresenze] = useState<any[]>([]);
// 5. Dettaglio goal fatti
...
const [partiteRigoriTirati, setPartiteRigoriTirati] = useState<any[]>([]);
  ...
  const fetchStatistiche = async (giocatoreUid: string, stagioneId: string, richiestaId: number) => {
  try {
...
if (allenamenti && allenamenti.length > 0) {
  ...
  saltati = allenamenti.filter(a => a.presente === false).length;
    ...
    if (richiestaId !== richiestaStatistiche.current) return;
    setStatistiche({
...
// 8. Recupera le partite in cui il giocatore risulta presente
...
// 9. Recupera le partite in cui il giocatore ha segnato
...
// 10. Recupera le partite in cui il giocatore ha fatto assist
...
// 11. Recupera i singoli rigori tirati dal giocatore
const fetchPartiteRigoriTirati = async (
  ...
  // 12. Lettura completa dello storico
  const leggiTutteLeRighe = async (creaQuery: () => any): Promise<any[]> => {
    const righe: any[] = [];
    const dimensionePagina = 500;
    while (true) {
      const { data, error } = await creaQuery().range(
        righe.length,
        righe.length + dimensionePagina - 1
      );
      if (error) throw error;
      if (!data?.length) return righe;
      righe.push(...data);
    }
  };

  // 13. Totali di tutte le stagioni dello stesso giocatore
  const fetchTutteLeStagioni = async (richiestaId: number) => {
    if (!id) return;
    const schede = await leggiTutteLeRighe(() => supabase
      .from('v_giocatori_completo')
      .select('*')
      .eq('giocatore_uid', id)
      .order('stagione_nome', { ascending: false })
      .order('giocatore_stagione_id', { ascending: true }));
    if (!schede.length) throw new Error('Nessuna stagione disponibile per questo giocatore.');
    const ids = [...new Set(schede.map((r) => r.giocatore_stagione_id))];
    const stagioniIds = [...new Set(schede.map((r) => r.stagione_id))];
    const [stats, voti, minuti, cartellini, rigoriPortiere, allenamenti,
      presenze, goal, assist, rigori] = await Promise.all([
      leggiTutteLeRighe(() => supabase.from('v_stat_giocatore_stagione')
        .select('goal_totali, presenze_totali, goal_subiti')
        .eq('giocatore_uid', id).in('stagione_id', stagioniIds).order('stagione_id')),
      leggiTutteLeRighe(() => supabase.from('voti_giocatori_media')
        .select('media_voto_mister, numero_voti_mister')
        .eq('giocatore_uid', id).in('stagione_id', stagioniIds).order('stagione_id')),
      leggiTutteLeRighe(() => supabase.from('minuti_giocati_totali')
        .select('tempo_giocato_sec').in('giocatore_stagione_id', ids).order('id')),
      leggiTutteLeRighe(() => supabase.from('cartellini')
        .select('tipo').in('giocatore_stagione_id', ids).in('stagione_id', stagioniIds).order('id')),
      leggiTutteLeRighe(() => supabase.from('rigori_partita')
        .select('esito').in('portiere_stagione_id', ids).in('stagione_id', stagioniIds).order('id')),
      leggiTutteLeRighe(() => supabase.from('allenamenti')
        .select('presente').eq('giocatore_uid', id).in('stagione_id', stagioniIds).order('id')),
      leggiTutteLeRighe(() => supabase
    .from("presenze")
    .select(`
      partita_id,
      partite (
    ...
    `)
    .in("giocatore_stagione_id", ids)
    .in("stagione_id", stagioniIds).order('partita_id').order('giocatore_stagione_id')),
      leggiTutteLeRighe(() => supabase
    .from("marcatori")
    .select(`
      partita_id,
    ...
    `)
    .in("giocatore_stagione_id", ids)
    .in("stagione_id", stagioniIds).order('id')),
      leggiTutteLeRighe(() => supabase
    .from("marcatori")
    .select(`
      partita_id,
    ...
    `)
    .in("assist_giocatore_stagione_id", ids)
    .in("stagione_id", stagioniIds).order('id')),
      leggiTutteLeRighe(() => supabase
    .from("rigori_partita")
    .select(`
    ...
    if (richiestaId !== richiestaStatistiche.current) return;
    const somma = (righe: any[], campo: string) =>
      righe.reduce((totale, riga) => totale + Number(riga[campo] ?? 0), 0);
    const votiValidi = voti.filter((v) => v.media_voto_mister != null && Number(v.numero_voti_mister) > 0);
    const numeroVoti = somma(votiValidi, 'numero_voti_mister');
    const rigoriSegnati = rigori.filter((r) => r.esito === 'segnato').length;
    const rigoriSbagliati = rigori.filter((r) => r.esito === 'sbagliato').length;
    const rigoriParati = rigoriPortiere.filter((r) => r.esito === 'sbagliato').length;
    const rigoriSubiti = rigoriPortiere.filter((r) => r.esito === 'segnato').length;
    setGiocatore(schede[0] as Giocatore);
    setStatistiche({
      goalTotali: somma(stats, 'goal_totali'),
      presenzeTotali: somma(stats, 'presenze_totali'),
      goalSubiti: somma(stats, 'goal_subiti'),
      mediaVotoMister: numeroVoti > 0
        ? votiValidi.reduce((totale, v) => totale + Number(v.media_voto_mister) * Number(v.numero_voti_mister), 0) / numeroVoti
        : 0,
      minutiGiocatiTotali: Math.floor(somma(minuti, 'tempo_giocato_sec') / 60),
      assistTotali: assist.length,
      gialliTotali: cartellini.filter((c) => c.tipo?.toLowerCase() === 'giallo').length,
      rossiTotali: cartellini.filter((c) => c.tipo?.toLowerCase() === 'rosso').length,
      rigoriTirati: rigoriSegnati + rigoriSbagliati,
      rigoriSegnati,
      rigoriSbagliati,
      rigoriRicevuti: rigoriParati + rigoriSubiti,
      rigoriParati,
      rigoriSubiti,
      allenamentiFatti: allenamenti.filter((a) => a.presente === true).length,
      allenamentiSaltati: allenamenti.filter((a) => a.presente === false).length,
    });
    setPartitePresenze(presenze.filter((r) => r.partite));
    setPartiteGoal(goal.filter((r) => r.partite));
    setPartiteAssist(assist.filter((r) => r.partite));
    setPartiteRigoriTirati(rigori.filter((r) => r.partite));
  };

  // 14. Caricamento della stagione selezionata
  const fetchGiocatore = async (stagioneId: string, richiestaId: number) => {
    if (stagioneId === TUTTE_LE_STAGIONI) {
      await fetchTutteLeStagioni(richiestaId);
      return;
    }
    if (!id) return;
...
if (stagioneIdDaState && stagioniUniche.find(s => s.id === stagioneIdDaState)) {
  ...
  // 15. Aggiornamento dello storico e protezione dei cambi stagione
  useEffect(() => {
    const richiestaId = ++richiestaStatistiche.current;
    if (stagioneSelezionata) {
      setLoadingStatistiche(true);
      fetchGiocatore(stagioneSelezionata, richiestaId)
        .catch((error) => {
          if (richiestaId !== richiestaStatistiche.current) return;
          console.error('Errore caricamento storico giocatore:', error);
          alert('Impossibile caricare tutte le statistiche. Riprova selezionando Tutte Le Stagioni.');
          setStagioneSelezionata(giocatore?.stagione_id || '');
        })
        .finally(() => {
          if (richiestaId === richiestaStatistiche.current) setLoadingStatistiche(false);
        });
    }
    return () => { richiestaStatistiche.current++; };
  ...
  if (authLoading || loading || loadingStatistiche || !giocatore) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#343434] via-[#404040] to-[#2b2b2b] px-[2px] pt-2 pb-6 box-border">
      <div className="w-full">
        ...
        {stagioniDisponibili.length > 0 && (
          <div className="mt-4 overflow-hidden rounded-xl border border-red-500 bg-white shadow-[0_4px_14px_rgba(0,0,0,0.22)]">
            <div className="flex items-center gap-3 px-4">
              ...
              >
                <option value={TUTTE_LE_STAGIONI}>Tutte Le Stagioni</option>
              ...
              <div className="border-t border-gray-200 bg-[#f7f7f7] px-4 py-3 space-y-2">
                {partitePresenze.length === 0 ? (
                  <div className="text-sm text-gray-500">
                    {stagioneSelezionata === TUTTE_LE_STAGIONI ? 'Nessuna presenza nelle stagioni disponibili.' : 'Nessuna presenza in questa stagione.'}
                  </div>
                ) : (
                  partitePresenze
                    .slice()
            ...
            {mostraGoal && (
              <div className="border-t border-gray-200 bg-[#f7f7f7] px-4 py-3 space-y-2">
                {partiteGoal.length === 0 ? (
                  <div className="text-sm text-gray-500">
                    {stagioneSelezionata === TUTTE_LE_STAGIONI ? 'Nessun goal nelle stagioni disponibili.' : 'Nessun goal in questa stagione.'}
                  </div>
                ) : (
                  Object.values(
            ...
            {mostraAssist && (
              <div className="border-t border-gray-200 bg-[#f7f7f7] px-4 py-3 space-y-2">
                {partiteAssist.length === 0 ? (
                  <div className="text-sm text-gray-500">
                    {stagioneSelezionata === TUTTE_LE_STAGIONI ? 'Nessun assist nelle stagioni disponibili.' : 'Nessun assist in questa stagione.'}
                  </div>
                ) : (
                  Object.values(
            ...
            {mostraRigoriTirati && (
              <div className="border-t border-gray-200 bg-[#f7f7f7] px-4 py-3 space-y-2">
                {partiteRigoriTirati.length === 0 ? (
                  <div className="text-sm text-gray-500">
                    {stagioneSelezionata === TUTTE_LE_STAGIONI ? 'Nessun rigore tirato nelle stagioni disponibili.' : 'Nessun rigore tirato in questa stagione.'}
                  </div>
                ) : (
                  partiteRigoriTirati
                    .slice()
        ...
        {/* 22. Media Voto Mister — visibile solo ad Admin o Creator */}
